...
export default function IntegrationPage() {
  ...
  async function handleSolve(e) {
    ...
    try {
      const res = await fetch(`${API}/api/integrate`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          variable,
          mode,
          lower: mode === "definite" ? lower : undefined,
          upper: mode === "definite" ? upper : undefined,
        }),
      });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      {/* Page header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Integration Solver</h1>
        <p className="mt-2 text-muted">
          Compute indefinite and definite integrals with step-by-step solutions.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <form onSubmit={handleSolve} className="rounded-2xl border border-border bg-white p-6 shadow-sm space-y-5">
            ...
            {/* Mode toggle */}
            <div>
              <label className="block text-sm font-medium mb-1.5">Integration Type</label>
              <div className="flex gap-2">
                {["indefinite", "definite"].map((m) => (
                  <button
                    key={m}
                    type="button"
                    onClick={() => setMode(m)}
            ...
            <div className="flex gap-4">
              <div>
                <label htmlFor="variable" className="block text-sm font-medium mb-1.5">
                  Variable
                </label>
                <select
                  id="variable"
              ...
              {/* Limits (definite only) */}
              {mode === "definite" && (
                <>
                  <div>
                    <label htmlFor="lower" className="block text-sm font-medium mb-1.5">Lower Limit</label>
                    <input
                      id="lower"
                      type="text"
                      value={lower}
                      ...
                      placeholder="0"
                      className="w-24 px-3 py-2.5 rounded-lg border border-border text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent/30"
                    />
                  </div>
                  <div>
                    <label htmlFor="upper" className="block text-sm font-medium mb-1.5">Upper Limit</label>
                    <input
                      id="upper"
                      type="text"
                      value={upper}
            ...
            <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
              ...
              {result.numericalResult && (
                <p className="mt-1 text-sm text-muted font-mono">
                  ≈ <MathDisplay value={result.numericalResult} />
                </p>
              )}
            </div>
          )}
          ...
          {result && (
            <GraphPlot
              ...
              labels={{ original: "f(x)", result: "∫f(x)dx" }}
            />
          )}
        </div>
